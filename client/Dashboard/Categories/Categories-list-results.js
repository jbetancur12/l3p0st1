      ...
      <PerfectScrollbar>
        <Box minWidth={1050} >
          <Table>
            ...
            <TableBody>
              {categories.slice(page * limit, page * limit + limit).map((provider) => (
                <TableRow
                  hover
                  key={provider._id}
                ...
                >
                  <TableCell padding='checkbox'>
                    <Checkbox
                      checked={selectedCategoryIds.indexOf(provider._id) !== -1}
                      onChange={(event) => handleSelectOne(event, provider._id)}
                      value='true'
                    />
                  </TableCell>
                  <TableCell>
